...
import edu.ie3.simona.service.primary.IntPrimaryServiceProxy.{IntPrimaryServiceStateData, SourceRef}
...
/** This actor has information on which models can be replaced by precalculated
  ...
  */
case class IntPrimaryServiceProxy(
                                   scheduler: ActorRef,
  ...
  override def onMessage(stateData: PrimaryServiceStateData): Receive = {
    stateData match {
      case IntPrimaryServiceStateData(
      modelToTimeSeries,
      timeSeriesToSourceRef,
      simulationStart,
      primaryConfig,
      mappingSource
      ) => onMessage(IntPrimaryServiceStateData(
        modelToTimeSeries,
        timeSeriesToSourceRef,
        simulationStart,
        primaryConfig,
   ...
   */
  private def onMessage(stateData: IntPrimaryServiceStateData): Receive = {
  ...
  override protected def handleCoveredModel(
    ...
    stateData match {
      case IntPrimaryServiceStateData(modelToTimeSeries, timeSeriesToSourceRef, simulationStart, primaryConfig, mappingSource) => {
            ...
            initializeWorker(
              ...
              case Success(workerRef) =>
                ...
                context become onMessage(
                  updateStateData(
                    IntPrimaryServiceStateData(modelToTimeSeries, timeSeriesToSourceRef, simulationStart, primaryConfig, mappingSource),
                    timeSeriesUuid,
                    workerRef)
                )
  ...
  private def updateStateData(
                               stateData: IntPrimaryServiceStateData,
                               timeSeriesUuid: UUID,
                               workerRef: ActorRef
                             ): IntPrimaryServiceStateData = {
...
object IntPrimaryServiceProxy {
  ...
  def props(
    ...
    new IntPrimaryServiceProxy(scheduler, initStateData, startDateTime)
  )
  ...
  /** Holding the state of an initialized proxy.
    ...
    *   The mapping source
    */
  final case class IntPrimaryServiceStateData(
