...
final case class ThermalHouse(
  ...
  /** Calculate the energy demand for heating at the instance in question by
    * calculating the energy needed to reach target temperature from actual
    * inner temperature. In case the inner temperature is at or below the lower
    * boundary temperature, this energy demand is interpreted as required
    * energy. Else, required energy will be zero. In case the inner temperature
    * is not at or above the target temperature, the demand is interpreted as
    * additional energy. Else, additional energy will be zero. The current
  ...
  def energyDemandDomesticHotWater(
      relevantData: HpRelevantData,
      currentThermalHouseState: Option[ThermalHouseState],
  ): ThermalEnergyDemand = {
    ...
    if (tick == 0) {
      ...
      currentThermalHouseState match {
        case Some(state) =>
          val lastStateTime =
