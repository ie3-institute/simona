...
/** A collection of methods used for solar irradiation calculation, as required
  * e.g. for [[PvModel]].
  */
object SolarIrradiationCalculation {
  ...
  /** Calculates the sunrise hour angle omegaSR given omegaSS.
    */
  def calcSunriseAngleOmegaSR(omegaSS: Angle): Angle = omegaSS * -1
    ...
    Radians(
      acos(
        sin(deltaInRad) * sin(latInRad) * cos(gammaInRad) -
          sin(deltaInRad) * cos(latInRad) * sin(gammaInRad) * cos(
            alphaEInRad
          ) +
          cos(deltaInRad) * cos(latInRad) * cos(gammaInRad) * cos(
            omegaInRad
          ) +
          cos(deltaInRad) * sin(latInRad) * sin(gammaInRad) *
          cos(alphaEInRad) * cos(omegaInRad) +
          cos(deltaInRad) * sin(gammaInRad) * sin(alphaEInRad) * sin(
            omegaInRad
          )
      )
    )
  ...
  def temperatureCorrectionFactor(time: ZonedDateTime): Double = {
    val tempCorr =
