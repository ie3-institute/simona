...
import edu.ie3.simona.event.listener.ResultEventListener.{AggregatedTransformer3wResult, Transformer3wKey}
...
object ExtResultEventListener {
  ...
  def apply(
           ...
           ): Behavior[ResultEvent | DelayedStopHelper.StoppingMsg] = Behaviors.receivePartial[ResultEvent | DelayedStopHelper.StoppingMsg] {
    case (_, ParticipantResultEvent(systemParticipantResult)) =>
      connection.queueExtResponseMsg(new ProvideResultEntities(systemParticipantResult))
      Behaviors.same

    case (ctx, PowerFlowResultEvent(nodeResults, switchResults, lineResults, transformer2wResults, transformer3wResults, congestionResults)) =>

      // handle all results except the three winding transformer results
      val results: Iterable[ResultEntity] = nodeResults ++ switchResults ++ lineResults ++ transformer2wResults ++ congestionResults
      connection.queueExtResponseMsg(new ProvideResultEntities(results.toList.asJava))


      // handling of three winding transformers
      val updatedResults = transformer3wResults.foldLeft(threeWindingResults) { case (allResults, result) =>
        ...
        partialResult.add(result).map { updatedResult =>
          if (updatedResult.ready) {
            ...
            updatedResult.consolidate.foreach(res => connection.queueExtResponseMsg(new ProvideResultEntities(res)))
    ...
    case (_, FlexOptionsResultEvent(flexOptionsResult)) =>
      connection.queueExtResponseMsg(new ProvideResultEntities(flexOptionsResult))
      Behaviors.same
