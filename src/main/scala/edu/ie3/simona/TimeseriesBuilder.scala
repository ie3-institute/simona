...
object TimeseriesBuilder {
  ...
  def load(start: ZonedDateTime): String = {
    ...
    hours.map(hour => s"2.7,1.31,${hour.format(DateTimeFormatter.ISO_DATE_TIME)}\n").foreach(loadBuilder.append)
  ...
  def noCM(start: ZonedDateTime): String = {
    ...
    hours.map(hour => s"0.0,${hour.format(DateTimeFormatter.ISO_DATE_TIME)}\n").foreach(stringBuilder.append)
  ...
  def eachHour(start: ZonedDateTime): String = {
    ...
    hours.zipWithIndex.map { case (hour, i) =>
      if (i % 2 == 0) {
  ...
  def mapping: Unit = {
    ...
    val jointGridContainer = CsvJointGridContainerSource.read("grid", ",", Path.of(".", "input", "ma_thesis", "fullGrid"), false)
    ...
    val mappingEntries = loads.map(load => new MappingEntry(load, loadSeries)) ++ feedIns.map(feedIn => new MappingEntry(feedIn, feedInSeries))

    val sink = new CsvFileSink(Path.of(".", "input", "ma_thesis", "fullGrid", "primary-x"))
