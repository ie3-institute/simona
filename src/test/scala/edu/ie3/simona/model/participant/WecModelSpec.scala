...
class WecModelSpec extends AnyFlatSpec with Matchers {
  ...
  def setupSpec(): Unit = {
    ...
    /** "WecModel" should "build correctly from input model" in { val wecModel =
      * buildWecModel() wecModel.uuid() shouldBe inputModel.uuid wecModel.id()
      * shouldBe inputModel.id wecModel.sRated() shouldBe
      * Sq.create(inputModel.getType.sRated.getValue.doubleValue(), Kilowatts)
      * wecModel.cosPhiRated() shouldBe inputModel.getType.cosPhiRated
      * wecModel.rotorArea() shouldBe
      * Sq.create(inputModel.getType.rotorArea.getValue.doubleValue(),
      * SquareMeters) wecModel.betzCurve() shouldBe new
      * WecModel.WecCharacteristic().apply(inputModel.getType.cpCharacteristic)
      * }
      */

    "WecModel" should "calculate active power output depending on velocity" in {
    ...
    "WecModel" should "calculate active power output depending on temperature" in {
      val wecModel = buildWecModel()
    ...
    /** "WecModel" should "determine Betz coefficient correctly" in { val
      * wecModel = buildWecModel() val velocities = Seq(2.0, 2.5, 18.0, 27.0,
      * 34.0, 40.0) val expectedBetzResults = Seq(0.115933516, 0.2010945555,
      * 0.108671106, 0.032198846, 0.000196644, 0.0)
      *
      * velocities.zip(expectedBetzResults).foreach { case (velocity,
      * betzResult) => val windVel = Sq.create(velocity, MetersPerSecond) val
      * betzFactor = wecModel.determineBetzCoefficient(windVel) val expected =
      * Sq.create(betzResult, Each) betzFactor shouldEqual expected } }
      */

    /** "WecModel" should "calculate air density correctly" in { val wecModel =
      * buildWecModel() val testCases = Seq( ( -15.0, 100129.44, 1.35121 ), (
      * -5.0, 99535.96, 1.29311 ), ( 0.0, 99535.96, 1.26944 ), ( 5.0, 100129.44,
      * 1.25405 ), ( 20.0, 100129.44, 1.18988 ), ( 25.0, 100427.25, 1.17341 ), (
      * 37.0, 100427.25, 1.12801 ), // test cases where no air pressure is given
      * ( 0.0, -1.0, 1.2041 ), ( 5.0, -1.0, 1.2041 ), ( 40.0, -1.0, 1.2041 ) )
      *
      * testCases.foreach { case (temperature, pressure, densityResult) => val
      * temperatureV = Sq.create(temperature, Celsius) val pressureV = if
      * (pressure > 0) Some(Sq.create(pressure, Pascals)) else Option.empty
      *
      * val airDensity = wecModel.calculateAirDensity(temperatureV,
      * pressureV).toKilogramsPerCubicMeter math.abs(airDensity - densityResult)
      * should be < TOLERANCE } }
      */
