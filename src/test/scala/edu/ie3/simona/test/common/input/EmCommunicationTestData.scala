...
import edu.ie3.simona.test.common.{ConfigTestData, DefaultTestData}
...
trait EmCommunicationTestData extends DefaultTestData with ConfigTestData {

  extension (node: NodeInput) {
    def toCoordinate: Coordinate = {
      val point = node.getGeoPosition
      Coordinate(point.getY, point.getX)
    }
  }
