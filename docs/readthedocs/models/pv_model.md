...
![](/model/pv_it_test_wholeyear.png){width="800"}

## Calculations

![](/model/pvradiation.png){.align-right width="600"} The energy produced by a photovoltaic (pv) unit in a specific time step is based on the diffuse and direct radiation provided by the used weather data. The following steps are done to calculate (= estimate) the power feed by the pv.

To calculate the overall feed in of the pv unit, the sum of the direct radiation, diffuse radiation and reflected radiation is needed. In the following, the formulas to calculate each of these radiations are presented and explained. The sections ends with the formula to calculate the the corresponding power feed in.
...
![Maximum and minimum value of declination angle](/model/declination_angle.png){.align-right}
...
**References:**
- ![Watter, Holger. (2013). Regenerative Energiesysteme: Grundlagen, Systemtechnik und Analysen ausgeführter Beispiele nachhaltiger Energiesysteme. 10.1007/978-3-658-23488-1.](/model/watter2015_chapter_photovoltaik.pdf){.align-center}\
...
$$ \omega_{SR}=\cos^{-1}(-tan (\phi) \cdot tan (\delta)\
$$
...
[![sun_angles_overview.png](/model/sun_angles_overview.png){.align-right width="400"}](https://www.itacanet.org/the-sun-as-a-source-of-energy/part-1-solar-astronomy/)
...
\*\*References:\*\*
- ![Zheng, H. (2017). Solar Energy Utilization and Its Collection Devices.](/model/zheng2017_solarenergyutilizationanditscollectiondevices.pdf){.align-center} p. 53, formula 2.3b\
